...
enum Emissiveness_type {COSINE, DISCRETE_BEAM_5, DISCRETE_BEAM_10,
                        DISCRETE_BEAM_30, UNIFORM, GAUSS_BEAM};
...
public:
  ...
  space_type refractive_index = 1.0;
  ...
  PBRMaterial(
      const PBRMaterial<space_type,color_type>& other) : albedo_const(other.albedo_const),
  ...
  space_type GetRoughnessSphere(
    ...
    return ExtractScalarFromTextureMap(
        point_on_surface,
        sphere_origin,
        roughness_uv_map_sphere.value().ey,
        *(roughness_map.get()));
  }
  ...
  space_type GetMetalnessSphere(
    ...
    return ExtractScalarFromTextureMap(
        point_on_surface,
        sphere_origin,
        metalness_uv_map_sphere.value().ey,
        *(metalness_map.get()));
  }
