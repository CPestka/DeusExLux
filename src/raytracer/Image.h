...
public:
  ...
  Pixel<color_type> operator+(const Pixel<color_type_other>& rhs) const noexcept {
    return Pixel<color_type>(Vec3<color_type>(channel) += rhs.channel);
  }
  template<Scalar_t color_type_other>
  ...
  template<Scalar_t color_type_other>
  Pixel<color_type> operator*(const Pixel<color_type_other>& rhs) const noexcept {
    return Pixel<color_type>(
        channel.x * rhs.channel.x,
        channel.y * rhs.channel.y,
        channel.z * rhs.channel.z);
  }
  template<Scalar_t color_type_other>
...
template<Scalar_t color_type1, Scalar_t color_type2>
Pixel<color_type1> operator+(color_type2 lhs, const Pixel<color_type1>& rhs) {
  return Pixel<color_type1>(rhs) + lhs; 
}
template<Scalar_t color_type1, Scalar_t color_type2>
Pixel<color_type1> operator-(color_type2 lhs, const Pixel<color_type1>& rhs) {
  return Pixel<color_type1>(rhs * static_cast<color_type2>(-1.0)) + lhs; 
}
template<Scalar_t color_type1, Scalar_t color_type2>
Pixel<color_type1> operator*(color_type2 lhs, const Pixel<color_type1>& rhs) {
  return Pixel<color_type1>(rhs) * lhs; 
