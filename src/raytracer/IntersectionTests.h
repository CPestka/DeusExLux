...
struct IntersectionResults {
  ...
  PBRMaterial<space_type, color_type>* hit_material;
};

template<Float_t space_type, Float_t color_type>
Pixel<color_type> GetAlbedo(
    const IntersectionResults<space_type,color_type>& intersection_results) {
  return intersection_results.hit_triangle ?
      intersection_results.hit_material->GetAlbedoTri(
          intersection_results.intersection_point,
          intersection_results.hit_triangle.value()) :
      intersection_results.hit_material->GetAlbedoSphere(
          intersection_results.intersection_point,
          intersection_results.hit_sphere_origin.value());
}

template<Float_t space_type, Float_t color_type>
Pixel<color_type> GetEmitance(
    const IntersectionResults<space_type,color_type>& intersection_results,
    space_type shading_angle) {
  return !intersection_results.hit_triangle ?
      intersection_results.hit_material->GetEmitanceSphere(
          intersection_results.intersection_point,
          intersection_results.hit_sphere_origin.value(),
          shading_angle) :
      intersection_results.hit_material->GetEmitanceTri(
          intersection_results.intersection_point,
          intersection_results.hit_triangle.value(),
          shading_angle);
}
...
template<Float_t space_type, Float_t color_type>
space_type GetMetalness(
    const IntersectionResults<space_type,color_type>& intersection_results) {
  return intersection_results.hit_triangle ?
      intersection_results.hit_material->GetMetalnessTri(
          intersection_results.intersection_point,
          intersection_results.hit_triangle.value()) :
      intersection_results.hit_material->GetMetalnessSphere(
          intersection_results.intersection_point,
          intersection_results.hit_sphere_origin.value());
}

template<Float_t space_type, Float_t color_type>
space_type GetRoughness(
    const IntersectionResults<space_type,color_type>& intersection_results) {
  return !intersection_results.hit_triangle ?
      intersection_results.hit_material->GetRoughnessSphere(
          intersection_results.intersection_point,
          intersection_results.hit_sphere_origin.value()) :
      intersection_results.hit_material->GetRoughnessTri(
          intersection_results.intersection_point,
          intersection_results.hit_triangle.value());
}

template<Float_t space_type, Float_t color_type>
space_type GetRefractiveIndex(
    const IntersectionResults<space_type,color_type>& intersection_results) {
  return !intersection_results.hit_triangle ?
      intersection_results.hit_material->GetRefractiveIndexSphere(
          intersection_results.intersection_point,
          intersection_results.hit_sphere_origin.value()) :
      intersection_results.hit_material->GetRefractiveIndexTri(
          intersection_results.intersection_point,
          intersection_results.hit_triangle.value());
}

template<Float_t space_type, Float_t color_type>
space_type GetTransmissity(
    const IntersectionResults<space_type,color_type>& intersection_results) {
  return intersection_results.hit_triangle ?
      intersection_results.hit_material->GetTransmissityTri(
          intersection_results.intersection_point,
          intersection_results.hit_triangle.value()) :
      intersection_results.hit_material->GetTransmissitySphere(
          intersection_results.intersection_point,
          intersection_results.hit_sphere_origin.value());
}

template<Float_t space_type, Float_t color_type>
Vec3<space_type> GetNormal(
    const IntersectionResults<space_type,color_type>& intersection_results) {
  return intersection_results.hit_triangle ?
      intersection_results.hit_material->GetNormalTri(
          intersection_results.intersection_point,
          intersection_results.hit_triangle.value()) :
      intersection_results.hit_material->GetNormalSphere(
          intersection_results.intersection_point,
          intersection_results.hit_sphere_origin.value());
}
...
void IntersectTriangle(const Tri<space_type,color_type>& tri,
  ...
  if (dist > epsilon && dist < distance_of_closest_hit_untill_now) {
    ...
    results.hit_material = tri.material.get();
...
void IntersectSphere(const Sphere<space_type,color_type>& sph,
  ...
  if (discriminant > 0) {
    ...
    if (dist > epsilon && dist < distance_of_closest_hit_untill_now) {
      ...
      distance_of_closest_hit_untill_now = dist;
      results.intersection_point = (dist * ray.direction) + ray.origin;
      results.hit_material = sph.material.get();
...
class BVHBoxHelper {
  ...
  BVHBoxHelper(
      BVHNode<space_type,color_type>* bvh_node,
      space_type dist) :
      bvh_node( bvh_node),
