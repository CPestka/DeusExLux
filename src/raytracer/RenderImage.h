...
Pixel<color_type> RenderPixel(
  ...
  for(uint32_t i=0; i<samples_per_pixel; i++){
    //Free AA yeahhhh
    space_type inter_pixel_delta_x = GetUniformRandom<space_type>();
    space_type inter_pixel_delta_y = GetUniformRandom<space_type>();
    ...
    //Depth of field :)
    space_type r = GetUniformRandom<space_type>(0.0, camera.lense_radius);
    space_type phi = GetUniformRandom<space_type>(0.0, 2 * M_PI);
    ...
    for(uint32_t i=y; i<=y_max; i++){
      for(uint32_t j=x; j<=x_max; j++){
        render.pixel_ptr[j + (camera.image_width * i)] +=
            RenderPixel(camera,j,i,samples_per_pixel,max_bounces,bvh,background);
      }
    }
  }
...
void ContinueRenderingImage(
  ...
  for(int i=0; i<thread_count-1; i++){
    render_threads.push_back(std::thread(
      RenderTiles<space_type,color_type>,
      std::ref(render),
      std::ref(camera),
      samples_per_pixel,
      max_bounces,
  ...
  std::cout << "Render completed in:\t" << time << "s" << std::endl;
}
...
Image<Pixel<color_type>> RenderImage(
  ...
  ContinueRenderingImage<space_type,color_type>(
      render,
      camera,
      samples_per_pixel,
      max_bounces,
